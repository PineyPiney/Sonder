...
class PlayerAnimations {
	companion object {
		...
		val playerIdleGen = Animation(
			"idleGen",
			20f,
			"characters/player/body/idle",
			(1..22).map { "frame_" + "%02d".format(it) },
			"characters/player/gen/idle_gen"
		)
		val playerWobbleGen = Animation(
			"wobbleGen",
			(1..22).associateWith {
				Animation.KeyFrame(
					mutableMapOf(
						"RND.txr" to "characters/player/body/idle/IMG_" + "%02d".format(
							it
						), "C2D.rtn" to Component.FloatField.float2String((it * 0.095238 * PI).f)
					)
				)
			}.toMutableMap(),
			12f,
			21,
			"characters/player/gen/wobble_gen"
		)
		val playerWalkGen = Animation(
			"walk",
			6f,
			"characters/player/body/walk",
			(0..6).map { "IMG_${2991 + it}" },
			"characters/player/gen/walk_gen"
		)
