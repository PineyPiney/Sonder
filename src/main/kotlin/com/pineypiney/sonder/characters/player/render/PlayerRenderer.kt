...
class PlayerRenderer : GameObject("player_renderer") {
	...
	var hair = object : GameObject() {
		...
		val texture = ColouredSpriteComponent(
			this,
			TextureLoader[ResourceKey("characters/player/hair/hair1")],
			600f,
			Vec4(0.2f, 0.08f, 0.02f, 1f),
			playerShader
		)
	...
	var hairColour
		get() = Vec3(hair.getComponent<ColouredSpriteComponent>()?.tint?.invoke() ?: Vec4())
		set(value) {
			hair.getComponent<ColouredSpriteComponent>()?.tint = { Vec4(value, 1f) }
		}
	var hairStyle
		get() = hair.getComponent<SpriteComponent>()?.sprite?.texture?.fileLocation?.removePrefix("characters\\player\\hair\\")
			?.substringBeforeLast('.') ?: ""
		set(value) {
			hair.getComponent<SpriteComponent>()?.sprite = Sprite(TextureLoader[ResourceKey(value)], 600f)
		}

	override fun addComponents() {
		super.addComponents()
		components.add(object : ColouredSpriteComponent(
			this@PlayerRenderer,
			TextureLoader[ResourceKey("characters/player/piccolo/walk/walk_right")],
			200f,
			Vec4(1f, 1f, 1f, 1f),
			playerShader
		) {
			override fun setUniforms() {
				super.setUniforms()
				uniforms.setVec2Uniform("origin") {
					Vec2(
						invFrames * ((floor(Timer.frameTime * animationSpeed * frames) % frames) + if (flipY) 1 else 0),
						0f
					)
				}
	...
	data class Walk(
		...
		) : this(dir, TextureLoader[ResourceKey("characters/player/piccolo/$tex")], frames, invWeight, speed, mirror)
	}
}