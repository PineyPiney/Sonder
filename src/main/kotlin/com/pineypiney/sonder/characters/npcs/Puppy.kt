...
class Puppy : GameObject() {
	...
	override fun addComponents() {
		...
		components.add(
			SpriteComponent(
				this,
				TextureLoader[ResourceKey("characters/puppy/idle/Untitled_Artwork-1")],
				600f
			)
		)
		...
		val idle = Animation(
			"idle",
			6f,
			"characters/puppy/idle",
			(1..4).map { "Untitled_Artwork-$it" },
			"characters/puppy/idle"
		)
		components.add(
			AnimatedComponent(
				this, idle, listOf(
					...
					Animation(
						"talk",
						6f,
						"characters/puppy/talk",
						(1..5).map { "Untitled_Artwork-$it" },
						"characters/puppy/talk"
					),
					Animation(
						"walk",
						6f,
						"characters/puppy/walk",
						(1..8).map { "Untitled_Artwork-$it" },
						"characters/puppy/walk"
					)
				)
			)
