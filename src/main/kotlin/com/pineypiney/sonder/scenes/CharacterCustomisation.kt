...
class CharacterCustomisation(engine: SonderEngine, val parent: SonderGameLogic) : SonderGameLogic(engine) {
	...
	override fun addObjects() {
		add(player)
		add(CharacterPart("skin_tones", Vec2(-0.9f, 0.3f), Vec2(0.9f)) {
			player.renderChild.skinTone = (Vec3.fromHex(it.parseInt(16)))
		})
		add(CharacterPart("hair_colours", Vec2(0.4f, 0.3f), Vec2(0.9f)) {
			player.renderChild.hairColour = (Vec3.fromHex(it.parseInt(16)))
		})
		add(CharacterPart("hair_styles", Vec2(0.4f, -0.4f), Vec2(0.9f)) {
			player.renderChild.hairStyle = "characters/player/hair/$it"
		})
		when (gameEngine.inputType) {
			ControlType.KEYBOARD -> add(
				TextButton("Done", Vec2(0.5f, -0.9f), Vec2(0.4f, 0.2f)) { _, _ -> saveAndExit() }
			)
	...
	fun selectPart(d: Int) {
		...
		if (parts.isNotEmpty()) {
			val selectedPart = parts.firstOrNull { it.parent.getComponent<HoverComponent>()?.shouldInteract() ?: false }
	...
	fun selectButton(d: Vec2) {
		val subMenu = parts.first {
			it.parent.getComponent<HoverComponent>()?.shouldInteract() == true
