...
class PauseMenu(engine: SonderEngine): SonderMenu(engine) {
    ...
    private val continueButton = TextButton("Continue", Vec2(-.4f, .5f), Vec2(.8f, .3f)){
        gameEngine.openGame(false)
    }

    private val exitButton = TextButton("Exit", Vec2(-.4f, -.8f), Vec2(.8f, .3f)){
    ...
    override fun onInput(state: InputState, action: Int): Int {
        ...
        if((state.i == GLFW.GLFW_KEY_ESCAPE || state == InputState(GLFW.GLFW_GAMEPAD_BUTTON_B, ControlType.GAMEPAD_BUTTON)) && action == GLFW.GLFW_PRESS){
            gameEngine.openGame(false)
            return INTERRUPT
        }
        else return action
