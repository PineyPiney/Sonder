...
class CharacterPartSubMenu(val settings: CharacterPart.CharacterMenuSettings, val category: MenuNode, val onClick: (String) -> Unit = {}) : MenuItem() {
    ...
    private fun createButtons(){
        ...
        val defaultButton: (Float) -> CharacterPartButton = getButton(category["button_prefab"] ?: "ColourButton")
        ...
        for (j in 0..<numbuttons)
        {
            ...
            val button = buttonPrefab(parent!!.getComponent<SpriteComponent>()!!.pixelsPerUnit)
    ...
    private fun positionButton(button: GameObject, i: Int) {
        ...
        button.position = Vec3(padX + column * settings.width, menuSize.y - (padY + buttonSize.y + row * settings.height), 0f)
    }

    private fun getButton(string: String) : (Float) -> CharacterPartButton =
        when(string){
