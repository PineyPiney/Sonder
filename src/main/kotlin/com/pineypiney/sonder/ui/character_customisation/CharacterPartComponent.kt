...
class CharacterPartComponent(parent: GameObject, val type: String, val onClick: (String) -> Unit) : Component(
    ...
    private fun createCategories(){
        ...
        menu.forEachIndexed{ i, category ->
            ...
            val hoverButton = HoverButton(Texture.broke, { (it.parent as HoverButton).selector?.active = true }){ if(parent.getComponent<DefaultInteractorComponent>()?.hover != true) (it.parent as HoverButton).selector?.active = false }
            parent.addChild(hoverButton)
            category.with("icon"){ icon -> hoverButton.icon = TextureLoader[ResourceKey("ui/character_customisation/$icon")]}
    ...
    fun getSelected(): HoverButton?{
        return parent.children.filterIsInstance<HoverButton>().firstOrNull { it.getComponent<HoverComponent>()?.hover == true }
    }
