...
class Phone(parent: GameObject, val game: SonderGamePlay) : Component(parent, "PHN"), PreRenderComponent,
	...
	val case = object : MenuItem() {
		...
		override fun addComponents() {
			...
			components.add(
				SpriteComponent(
					this,
					TextureLoader[ResourceKey("ui/phone/case")],
					ppu,
	...
	fun position() {
		...
		if (orientationStage == 0f) {
			parent.position = Vec3(
				(.43f * case.renderer!!.renderSize.x) - SonderWindow.INSTANCE.aspectRatio,
				(((openStage.serp() * .8f) - .4f) * case.renderer!!.renderSize.y) - 1f,
				parent.position.z
			)
			...
			parent.position = Vec3(
				(Vec2(.43f, .4f) * case.renderer!!.renderSize - Vec2(
					SonderWindow.INSTANCE.aspectRatio,
					1f
				)) * (1.0 - orientationStage), parent.position.z
			)
