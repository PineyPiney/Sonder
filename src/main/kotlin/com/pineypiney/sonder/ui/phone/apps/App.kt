...
abstract class App(parent: GameObject, val phone: Phone): DefaultInteractorComponent(parent, "APP"), UpdatingComponent {
	...
	val background = object : SpriteComponent(MenuItem(), phone.screen.parent.getComponent<SpriteComponent>()!!.texture, Phone.ppu, ShaderLoader[ResourceKey("vertex/menu"), ResourceKey("fragment/phone/app_background")]){
	...
	override fun onInput(window: WindowI, input: InputState, action: Int, cursorPos: Vec2): Int {
		...
		if(action == 1){
			when(input){
				InputState(GLFW.GLFW_KEY_ESCAPE), InputState(GLFW.GLFW_GAMEPAD_BUTTON_B, ControlType.GAMEPAD_BUTTON) -> {
					if(state and 2 > 0) {
...
if(state and 1 > 0 && fill() == 1f) endOpen()
	...
	fun fill(): Float{
		return (if(appOpenTime > 0) (Timer.frameTime - appOpenTime) * Phone.orientationSpeed else 1.0 - ((Timer.frameTime + appOpenTime) * Phone.orientationSpeed)).coerceIn(0.0, 1.0).toFloat()
	}
}